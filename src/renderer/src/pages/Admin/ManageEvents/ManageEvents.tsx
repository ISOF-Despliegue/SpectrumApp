...
const REWARD_CODE_PATTERN = /^[A-Z0-9]{4}(?:-[A-Z0-9]{4}){3}$/;
...
const normalizeDateTimeLocalValue = (value: string | null | undefined, fallback = new Date()): string => {
  const normalizedValue = (value ?? '').trim();

  if (DATE_TIME_LOCAL_PATTERN.test(normalizedValue)) {
    return normalizedValue;
  }

  const parsedDate = new Date(normalizedValue);
  return toDateTimeLocalValue(isValidDate(parsedDate) ? parsedDate : fallback);
};

const normalizeRewardCode = (code: string | null | undefined): string => (code ?? '').trim().toUpperCase();
...
const createEmptyForm = (): DropEventPayload => {
  ...
  return {
    ...
    gameTitle: '',
    platform: 'PC',
    startAt: toDateTimeLocalValue(new Date(now.getTime() + 60 * 60 * 1000)),
    joinDeadlineAt: toDateTimeLocalValue(new Date(now.getTime() + 2 * 60 * 60 * 1000)),
    revealAt: toDateTimeLocalValue(new Date(now.getTime() + 3 * 60 * 60 * 1000)),
    endAt: toDateTimeLocalValue(new Date(now.getTime() + 4 * 60 * 60 * 1000)),
    totalSlots: 100,
    publicChallengeCode: '',
    accessKeys: []
  };
...
const toPayload = (form: DropEventPayload): DropEventPayload => ({
  ...
  startAt: new Date(normalizeDateTimeLocalValue(form.startAt)).toISOString(),
  joinDeadlineAt: new Date(normalizeDateTimeLocalValue(form.joinDeadlineAt)).toISOString(),
  revealAt: new Date(normalizeDateTimeLocalValue(form.revealAt)).toISOString(),
  endAt: new Date(normalizeDateTimeLocalValue(form.endAt)).toISOString(),
  publicChallengeCode: '',
  accessKeys: form.accessKeys.map(normalizeRewardCode).filter(Boolean)
});
...
const fromEvent = (event: DropEvent): DropEventPayload => ({
  ...
  startAt: normalizeDateTimeLocalValue(event.startAt),
  joinDeadlineAt: normalizeDateTimeLocalValue(event.joinDeadlineAt),
  revealAt: normalizeDateTimeLocalValue(event.revealAt),
  endAt: normalizeDateTimeLocalValue(event.endAt),
...
export const AdminManageEvents = (): React.JSX.Element => {
  ...
  const updateField = (field: keyof DropEventPayload, value: string | number): void => {
    setForm(current => ({ ...current, [field]: value }));
  };

  const renderDateTimeField = (
    field: 'startAt' | 'joinDeadlineAt' | 'revealAt' | 'endAt',
    label: string
  ): React.JSX.Element => (
    <label className={styles.dateTimeField}>
      ...
      <input
        type="datetime-local"
        step="60"
        value={normalizeDateTimeLocalValue(String(form[field]))}
        onChange={event => updateField(field, event.target.value)}
        onBlur={event => updateField(field, normalizeDateTimeLocalValue(event.target.value))}
      />
    </label>
  );
  ...
  const validateForm = (): string | null => {
    ...
    if (accessKeys.some(code => !REWARD_CODE_PATTERN.test(code))) {
      return t('manageEvents.validation.codeFormat');
    }
    ...
    <div className={styles.page}>
      ...
      <section className={styles.formPanel}>
        ...
        <div className={styles.formGrid}>
          <input
            placeholder={t('manageEvents.labels.title')}
          ...
          <div className={styles.autocomplete}>
            <input
              placeholder={t('manageEvents.labels.game')}
              maxLength={FIELD_LIMITS.shortText}
              value={form.gameTitle}
              onChange={event => updateField('gameTitle', event.target.value)}
            />
          ...
          <textarea
            placeholder={t('manageEvents.labels.description')}
          ...
          <textarea
            placeholder={t('manageEvents.labels.codes')}
            maxLength={FIELD_LIMITS.dropCodes}
            value={accessKeysText}
            onChange={event => setAccessKeysText(event.target.value.toUpperCase())}
          />

          <input
            aria-label={t('manageEvents.labels.totalSlots')}
            type="number"
            min="1"
            value={form.totalSlots}
            onChange={event => updateField('totalSlots', Number(event.target.value))}
          />

          {renderDateTimeField('startAt', t('manageEvents.labels.startAt'))}
          {renderDateTimeField('joinDeadlineAt', t('manageEvents.labels.joinDeadlineAt'))}
          {renderDateTimeField('revealAt', t('manageEvents.labels.revealAt'))}
          {renderDateTimeField('endAt', t('manageEvents.labels.endAt'))}
        </div>

        <div className={styles.actions}>
          <button onClick={handleSubmit} disabled={isLoading}>
            {editingId ? t('manageEvents.saveChanges') : t('manageEvents.create')}
          </button>
        ...
        {events.map(event => (
          <article className={styles.eventCard} key={event.eventId}>
            ...
            <div className={styles.eventBody}>
              ...
              <dl>
                <div>
                  <dt>{t('manageEvents.labels.gameLabel')}</dt>
                  <dd>{event.gameTitle}</dd>
                </div>

                <div>
                  <dt>{t('manageEvents.labels.platform')}</dt>
                  <dd>{event.platform}</dd>
                </div>

                <div>
                  <dt>{t('manageEvents.labels.slots')}</dt>
                  <dd>
                ...
                <div>
                  <dt>{t('manageEvents.labels.codesCount')}</dt>
                  <dd>
                    {event.rewardCodesAvailable ?? event.keysAvailable}/{event.rewardCodesTotal ?? event.keysTotal}
                  </dd>
                </div>

                <div>
                  <dt>{t('manageEvents.labels.winners')}</dt>
                  <dd>
                    {event.winners?.map(winner => winner.username).join(', ') ||
                      event.winnerUsername ||
                      t('manageEvents.pending')}
                  </dd>
                </div>
              </dl>
